...
@test "ramalama --dry-run run basic output" {
    model=tiny
    image=m_$(safename)
	...
	run_ramalama --dry-run run ${model}
	...
	run_ramalama --dry-run run --seed 9876 -c 4096 --name foobar ${model}
	...
	run_ramalama --dry-run run --name foobar ${model}
	is "$output" "${verify_begin} foobar .*" "dry-run correct with --name"
	...
	RAMALAMA_IMAGE=${image}:1234 run_ramalama --dry-run run ${model}
	is "$output" ".*${image}:1234 llama-run" "verify image name"
    else
	run_ramalama --dry-run run -c 4096 ${model}
	is "$output" 'llama-run -c 4096 --temp 0.8.*/path/to/model.*' "dry-run correct"
...
@test "ramalama --dry-run run ensure env vars are respected" {
    skip_if_nocontainer
    model=tiny

    ASAHI_VISIBLE_DEVICES=99 run_ramalama --dry-run run ${model}
    is "$output" ".*-e ASAHI_VISIBLE_DEVICES=99" "ensure ASAHI_VISIBLE_DEVICES is set from environment"

    CUDA_LAUNCH_BLOCKING=1 run_ramalama --dry-run run ${model}
    is "$output" ".*-e CUDA_LAUNCH_BLOCKING=1" "ensure CUDA_LAUNCH_BLOCKING is set from environment"

    HIP_VISIBLE_DEVICES=99 run_ramalama --dry-run run ${model}
    is "$output" ".*-e HIP_VISIBLE_DEVICES=99" "ensure HIP_VISIBLE_DEVICES is set from environment"

    HSA_OVERRIDE_GFX_VERSION=0.0.0 run_ramalama --dry-run run ${model}
    is "$output" ".*-e HSA_OVERRIDE_GFX_VERSION=0.0.0" "ensure HSA_OVERRIDE_GFX_VERSION is set from environment"

    HIP_VISIBLE_DEVICES=99 HSA_OVERRIDE_GFX_VERSION=0.0.0 run_ramalama --dry-run run ${model}
    is "$output" ".*-e HIP_VISIBLE_DEVICES=99" "ensure HIP_VISIBLE_DEVICES is set from environment"
    is "$output" ".*-e HSA_OVERRIDE_GFX_VERSION=0.0.0" "ensure HSA_OVERRIDE_GFX_VERSION is set from environment"
}
